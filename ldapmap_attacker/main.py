...
Placeholders: Use INPUT in the URL or data to indicate the injection point.
    ...
    parser.add_argument(
        "--force",
        action="store_true",
        help="Skip LDAP validation and force testing (may cause false positives)"
    )
    parser.add_argument(
        "--ignore-backend-proof",
        action="store_true",
        help="Skip backend proof validation (dangerous, may target fake APIs)"
    )
    parser.add_argument(
        "--max-payloads",
        type=int,
        default=None,
        help="Limit number of payloads to test (ex: 100 for quick scan)"
    )
    parser.add_argument(
        "--retries",
        type=int,
    ...
    exploitation.add_argument(
        "--dump",
        action="store_true",
        help="Full LDAP database dump (requires --confirm-dump)"
    )
    exploitation.add_argument(
        "--confirm-dump",
        action="store_true",
        help="Acknowledge authorization for aggressive dump operation"
    )
    exploitation.add_argument(
        "--export",
...
def main():
    ...
    session = Session(
        ...
        ignore_backend_proof=args.ignore_backend_proof,
    )
    ...
    try:
        ...
        if result is False:
            logger.error("Scan aborted - target does not appear to be LDAP-based")
            logger.error("Use --force to override and test anyway (not recommended)")
            sys.exit(1)
        ...
        if args.exploit or args.enum_users or args.dump or args.groups or args.brute_attrs or args.detect_server or args.fuzz or args.blind:
            if engine.has_vulnerabilities() or args.blind:
                print()
                print("=" * 60)
                print("PHASE 2: CAPABILITY ASSESSMENT")
                print("=" * 60)
                print()
                logger.info("Determining what exploitation techniques are applicable...")
                logger.warning("[!] This step validates what the injection can actually do")
                logger.warning("[!] Not all detected injections are exploitable for data extraction")
                print()

                capabilities = engine.assess_capabilities()

                if not capabilities:
                    logger.error("BACKEND VALIDATION FAILED")
                    logger.error("Cannot prove this is a real LDAP backend")
                    logger.error("Target responses don't contain LDAP structures (DN, objectClass, etc.)")
                    logger.error("This may be:")
                    logger.error("  - A mock API with fake data")
                    logger.error("  - Frontend rendering without LDAP queries")
                    logger.error("  - WAF/sanitization blocking LDAP reflection")
                    logger.error("Use --ignore-backend-proof to skip this check (not recommended)")
                    sys.exit(1)

                print()
                print("Detected capabilities:")
                for cap, desc in capabilities.items():
                    status = "[OK]" if desc.get("available") else "[NO]"
                    print(f"  {status} {cap}: {desc.get('note', 'N/A')}")
                print()

                if not any(desc.get("available") for desc in capabilities.values()):
                    logger.error("No usable exploitation capabilities found")
                    logger.error("The target may have proper input validation or limited LDAP exposure")
                    sys.exit(1)

                logger.warning("=" * 60)
                logger.warning("IMPORTANT: Before proceeding to extraction")
                logger.warning("=" * 60)
                logger.warning("1. Verify this is a REAL LDAP backend, not a mock API")
                logger.warning("2. Check server logs to confirm actual LDAP queries")
                logger.warning("3. Extracted 'passwords' in labs are often fake/mock data")
                logger.warning("4. Real LDAP directories rarely expose passwords via injection")
                print()

                if args.dump and not args.confirm_dump:
                    logger.error("[SECURITY] Full dump requires explicit confirmation")
                    logger.error("This operation is aggressive and may be illegal without authorization")
                    logger.error("Use --confirm-dump to acknowledge you have proper authorization")
                    sys.exit(1)

                logger.success("Starting controlled exploitation...")
